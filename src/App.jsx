  ...
  const handleImageUpload = async (e) => {
    try {
      ...
      if (!e.target.files || e.target.files.length === 0) {
        throw new Error('Sélectionnez une image depuis la galerie.');
      }
      ...
      if (!data || !data.publicUrl) throw new Error("Erreur de lien public.");
    ...
    <div className="min-h-screen bg-[#FAF9F6] text-brandBlack antialiased font-sans">
      ...
      <nav className={`sticky ${isAdmin ? 'top-[38px]' : 'top-0'} z-40 bg-white/90 backdrop-blur-md border-b border-gray-100 px-4`}>
        <div className="max-w-7xl mx-auto h-16 flex items-center justify-between">
          ...
          {/* Bouton Burger à gauche pour Mobile */}
          <button 
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            className="p-1 text-gray-700 hover:text-gold md:hidden"
          >
            <Menu size={24} />
          </button>

          {/* Logo Central (Mobile) / Gauche (Desktop) */}
          ...
          {/* Menu Classique (Masqué sur Mobile) */}
        ...
        {/* Menu Déroulant Latéral Mobile */}
        {isMobileMenuOpen && (
          <div className="fixed inset-0 bg-black/50 z-50 md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
            <div className="w-64 bg-white h-full p-6 space-y-8 flex flex-col justify-between shadow-2xl animate-in slide-in-from-left duration-200" onClick={e => e.stopPropagation()}>
              ...
              {['Tous', 'Homme', 'Femme'].map((sub) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-[11px] font-semibold tracking-wide uppercase whitespace-nowrap transition ${
                    selectedSubcategory === sub ? 'bg-brandBlack text-white' : 'bg-gray-100 text-gray-600'
                  }`}
                >
          ...
          <div className="bg-white rounded-2xl max-w-sm w-full p-5 relative shadow-xl my-auto">
            ...
            <form onSubmit={handleSaveProduct} className="space-y-3.5">
              ...
              <div>
                ...
                <label className="flex items-center justify-center gap-1.5 border border-dashed border-gray-300 rounded-xl py-2 px-3 text-xs font-semibold text-gray-600 bg-gray-50 cursor-pointer hover:bg-gray-100 transition">
                  <Upload size={14} className="text-gray-400" />
                  <span>{uploading ? 'Téléchargement...' : 'Ouvrir la Galerie'}</span>
                  <input type="file" accept="image/*" onChange={handleImageUpload} disabled={uploading} className="hidden" />
                </label>
                {formData.image_url && <p className="text-[10px] text-green-600 mt-1 truncate">✓ Image prête</p>}
              </div>
              <div>
              ...
              <button type="submit" disabled={uploading} className={`w-full bg-gold text-white py-2.5 rounded-xl font-bold text-xs ${uploading ? 'opacity-50' : 'hover:bg-brandBlack'}`}>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-3">
            ...
            <div className="flex gap-4 pt-2">
              <a href={INSTAGRAM_URL} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-gold transition">
                <Instagram size={20} />
              </a>
              <a href={TIKTOK_URL} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-gold transition">
                <TikTokIcon size={20} />
              </a>
            </div>
          </div>
